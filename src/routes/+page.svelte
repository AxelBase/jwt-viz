<script>
	...
	// JSON-LD for WebSite / Organization (basic)
	const siteJsonLd = {
...
<div>
	...
	{:else if decoded.errors.length > 0}
		<div class="alert alert-danger mt-3">
			{#each decoded.errors as err}<div>{err}</div>{/each}
		</div>
	{/if}
</div>
...
<div class="container mt-5">
	<section id="about" class="page-section mb-5">
		...
		<p>
			...
			JWT Viz runs
			<strong>100% in your browser</strong>, ensuring <strong>zero data transmission</strong>,
			<strong>no logging</strong>, and
			<strong>complete isolation</strong>
			of your tokens.
		</p>
		<br />
		<p>
			Designed with modern web standards and performance in mind, JWT Viz leverages
		...
		<p>
			...
			or even install the app as a
			<strong>Progressive Web App (PWA)</strong>
			for instant access on desktop or mobile, even without an internet connection.
		</p>
		<br />
		<p>Key features include:</p>
		<ul>
			<li>
				<strong>Dual-panel tree view</strong>
				of header and payload with expand/collapse and search
			</li>
			<li><strong>Real-time syntax-highlighted JSON</strong> and encoded segment display</li>
			<li>
				<strong>Client-side signature verification</strong>
				for HS256 and RS256 using public keys or secrets
			</li>
			<li>
			...
			<li>
				<strong>Standard claim highlighting</strong>
				(e.g., <code>exp</code>, <code>iss</code>) with expiration warnings
			</li>
		</ul>
		<p>
			Built as a <strong>static site</strong>, JWT Viz generates a single <code>index.html</code>
			with bundled assets—making it ideal for internal tools, documentation, or secure environments.
			Host it on any static file server, share via USB, or embed in Confluence. No backend, no database,
			no risk.
		</p>
		<p>
			...
			make JWT inspection
			<strong>accessible, secure, and educational</strong>
			for everyone in the web security ecosystem.
		</p>
		<p class="italic-note">
	...
	<section id="how-to-use" class="page-section mb-5">
		...
		<p>
			<strong>Paste</strong>
			a JWT into the large text area at the top, or
			<strong>drag and drop</strong>
			a <code>.txt</code>
			or <code>.jwt</code> file directly onto the input box. The tool instantly parses the token and
			displays any errors (e.g., malformed format, invalid Base64).
		</p>
		...
		<p>
			...
			HS256) or <strong>public key in PEM/SPKI format</strong>
			(for RS256). The tool uses the Web Crypto API to verify the signature
			<strong>in-browser</strong>. Results show:
		</p>
		<ul>
	...
	<section id="faq" class="page-section">
		...
		<div class="faq-accordion">
			<details>
				...
				<div class="faq-content">
					<p>
						<strong>No.</strong>
						JWT Viz is <strong>100% client-side</strong>. All parsing, decoding, visualization, and
						signature verification happen in your browser using JavaScript and Web Crypto. No data
						is transmitted, logged, or stored. You can verify this by checking network requests or
						running it offline.
					</p>
				</div>
			</details>
			...
			<details>
				...
				<div class="faq-content">
					<p>
						Yes. After the first load, the PWA caches all assets. You can copy the
						<code>build/</code>
			...
			<details>
				...
				<div class="faq-content">
					<p>
						<strong>HS256</strong>
						(HMAC-SHA256) and <strong>RS256</strong> (RSA-SHA256). These cover ~95% of real-world use
						cases. ES256 and others are not yet supported due to Web Crypto limitations.
					</p>
				</div>
			</details>
			...
			<details>
				...
				<div class="faq-content">
					<p>
						...
						(HS256) should be used in the browser. Private keys should <strong>never</strong>
						leave secure environments. JWT Viz enforces this pattern.
					</p>
				</div>
			</details>
		...
		<p class="italic-note">
			<em>Still have questions? The source code is your documentation.</em>
		</p>
	</section>
</div>
